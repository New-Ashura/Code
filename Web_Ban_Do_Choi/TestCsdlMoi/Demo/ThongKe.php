...
$conn = $conn =new mysqli('localhost','root','','Web_do_choi');
    ...
    //$sql = "SELECT DATE(NgayLap) AS Ngay, SUM(TongTien) AS TongTien FROM HoaDon GROUP BY DATE(NgayLap)";
    // $sql = "SELECT WEEKDAY(NgayLap) AS NgayTrongTuan, SUM(TongTien) AS TongTien 
    //     FROM HoaDon 
    //     GROUP BY WEEKDAY(NgayLap)";
    $sql = "SELECT DATE_FORMAT(NgayLap, '%Y-%m') AS Thang, SUM(TongTien) AS TongTien FROM HoaDon GROUP BY DATE_FORMAT(NgayLap, '%Y-%m')";
